...
class DeepKernelSUQ:
    ...
    def get_posterior_with_mapping_encoding(self, x, y):
        partial = x[:, self.point_cloud.shape[1]:, :]
        partial = partial.to(self.device)
        # compute encoding in a batch
        encoding = self.encoder(partial.transpose(1, 2))
        ...
        for i in range(bs):
            encoded_mapping = self.map_norm(self.cov_network(encoded_x[i]))
            cov_matrix = self.covar_after_mapping(encoded_mapping).evaluate_kernel().to_dense().to(self.device)
            kernel_ff = cov_matrix[:self.point_cloud.shape[1], :self.point_cloud.shape[1]]
            kernel_pf = cov_matrix[self.point_cloud.shape[1]:, :self.point_cloud.shape[1]]
            kernel_pp = cov_matrix[self.point_cloud.shape[1]:, self.point_cloud.shape[1]:]
            additional_noise = self.noise_var * torch.eye(self.partial_cloud.shape[1]).to(self.device)
    ...
    def get_posterior_diagonal(self, x, y):
        ...
        posterior_nlls = torch.empty(bs).to(self.device)
        for i in range(bs):
            mapped_cloud = self.map_norm(self.cov_network(encoded_x[i]))
    ...
    def train_with_mapping_encoding(self, num_epochs=20, batch_size=16, print_every=1, learning_rate=0.0005, weight_decay=1e-5):
        train_x = torch.cat((self.point_cloud, self.partial_cloud), 1).to(self.device)
        num_batches = np.ceil(train_x.size(0) / batch_size).astype('int')
        optimizer = torch.optim.AdamW([
            {'params': self.encoder.parameters()},
        ...
        for i in range(num_epochs):
            for j in range(num_batches):
                if j < num_batches-1:
                    ...
                    y = self.partial_value_train[(num_batches-1)*batch_size:].to(self.device)
                optimizer.zero_grad()
                output = self.get_posterior_with_mapping_encoding(x, y)
                # print(output)
                loss = torch.mean(output)
                loss.backward()
                optimizer.step()
    ...
    def train_cholesky_plus_noise(self, num_epochs=20, batch_size=16, print_every=1, learning_rate=0.0005, weight_decay=1e-5):
        train_x = torch.cat((self.point_cloud, self.partial_cloud), 1).to(self.device)
        num_batches = np.ceil(train_x.size(0) / batch_size).astype('int')
        optimizer = torch.optim.AdamW([
            {'params': self.encoder.parameters()},
        ...
        for i in range(num_epochs):
            for j in range(num_batches):
                if j < num_batches - 1:
                    ...
                    y = self.partial_value_train[(num_batches - 1) * batch_size:].to(self.device)
                optimizer.zero_grad()
                output = self.get_posterior_cholesky_plus_noise(x, y)
                # print(output)
                loss = torch.mean(output)
                loss.backward()
                optimizer.step()
    ...
    def predict(self, points_to_predict=None, do_mapping=False, encoding_only=False):
        ...
        for i in range(bs):
            ...
            if do_mapping:
                if encoding_only:
                    ...
                    cov_matrix = self.covar_after_mapping(mapped_cloud).evaluate_kernel().to_dense().to(
                        self.device)
                else:
                    mapped_cloud = self.cov_network(self.enc_norm(encoded_points[i]))
                    cov_matrix_data = self.covar_module_data(self.data_norm(test_x[i])).evaluate_kernel().to_dense().to(
                        self.device)
                ...
                cov_matrix = self.covar_module_data(mapped_cloud).evaluate_kernel().to_dense().to(self.device)
